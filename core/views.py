...
def like_profile(request):
    """Handle liking/passing a profile"""
    try:
        ...
        if existing_like:
            return JsonResponse({'error': 'Already rated this user'}, status=400)
        ...
        cache.delete(f'user_matches_{request.user.id}')
        cache.delete(f'potential_matches_{request.user.id}')
        cache.delete(f'user_matches_{to_user.id}')
        cache.delete(f'potential_matches_{to_user.id}')
            ...
            if mutual_like:
                ...
                async_to_sync(channel_layer.group_send)(
                    f'notifications_{to_user.id}',
                    {
                        ...
                        'message': f'You have a new match with {request.user.username}!',
                ...
                async_to_sync(channel_layer.group_send)(
                    ...
                    {
                        'type': 'match_notification',
                        'message': f'You have a new match with {to_user.username}!',
